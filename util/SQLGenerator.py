...
class SQLGenerator:
    ...
    TEMPLATE_SQL_SEARCH_BY_FIELD = "SELECT * FROM #TABLE# WHERE #FIELD# = #FIELD_VALUE#"
    TEMPLATE_SQL_SEARCH_BY_FIELD_WITH_INNER_JOIN = "SELECT * FROM #TABLE_A# T1 INNER JOIN #TABLE_B# T2 ON (T1.#ID_A# = T2.#ID_B#) WHERE T1.#FIELD# = #FIELD_VALUE#"
    ...
    @staticmethod
    def generator_sql_search_by_field(table, field, field_value):
        sql = SQLGenerator.TEMPLATE_SQL_SEARCH_BY_FIELD

        sql = sql.replace("#TABLE#", table)
        sql = sql.replace("#FIELD#", field)
        sql = sql.replace("#FIELD_VALUE#", str(field_value))

        return sql

    def generator_sql_search_by_field_with_inner_join(table, field, field_value):
        sql = SQLGenerator.TEMPLATE_SQL_SEARCH_BY_FIELD
